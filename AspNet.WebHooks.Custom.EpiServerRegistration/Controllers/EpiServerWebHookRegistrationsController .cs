using AspNet.WebHooks.Custom.EpiServerRegistration.Filters;
using AspNet.WebHooks.Custom.EpiServerRegistration.Models;
using AspNet.WebHooks.Custom.EpiServerRegistration.Routes;
...
namespace AspNet.WebHooks.Custom.EpiServerRegistration.Controllers
{
    ...
    /// </summary>
    [Authorize]
    public class EpiServerWebHookRegistrationsController : ApiController
    {
        ...
        [ResponseType(typeof(WebHook))]
        public async Task<IHttpActionResult> Subscribe(SubscriptionRequest request)
        {
            WebHook webHook = new WebHook()
            { Description = request.Description, WebHookUri = string.Format("{0}/api/webhooks/incoming/custom", request.WebURI), Secret = request.Secret};
            if (!string.IsNullOrEmpty(request.Services))
            {
                foreach (string filter in request.Services.Split(','))
                {
                    webHook.Filters.Add(filter.Trim());
                }
            }
            ...
            try
            {
                ...
                if (result == StoreResult.Success)
                {
                    return CreatedAtRoute("Subscribe", new { id = webHook.Id }, webHook);
                }
                return CreateHttpResult(result);
            }
            catch (Exception ex)
